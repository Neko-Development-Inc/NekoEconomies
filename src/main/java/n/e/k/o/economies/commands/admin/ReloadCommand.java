...
public class ReloadCommand implements Command<CommandSource> {
    ...
    public ReloadCommand(NekoEconomies nekoEconomies, UserManager userManager, EconomiesManager economiesManager, IStorage storage, CommandHelper commandHelper, Config config, Logger logger) {
    ...
    public int run(CommandContext<CommandSource> ctx) throws CommandSyntaxException {
        ...
        if (!commandHelper.canExecuteCommand(source, config.settings.permissions.reload, true))
            return SINGLE_SUCCESS;

        source.sendFeedback(StringColorUtils.getColoredString("ReloadCommand command"), true);
